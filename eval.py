...
def evaluate_model(
    ...
    _, _, _, species_labels, _ = manifest_generator_wrapper(
        dominant_threshold, export=True
    )
    ...
    _, val, _ = datacreator.create_dataset(start_rank)
    ...
    try:
        ...
        if hasattr(val_dataset, "dataset") and hasattr(
            val_dataset.dataset, "dominant_class_idx"
        ):
            # This is our BinaryInsectDataset
            binary_dataset = val_dataset.dataset
            dominant_class_name = binary_dataset.dataset.classes[
                binary_dataset.dominant_class_idx
            ]
            class_names = [dominant_class_name, "Other Species"]
            print("Detected binary classification dataset:")
            ...
            dominant_class_name = val_dataset.dataset.classes[
                val_dataset.dominant_class_idx
            ]
            class_names = [dominant_class_name, "Other Species"]
            print("Detected binary classification dataset:")
            print(f"  Class 0: {class_names[0]} (dominant species)")
            ...
            if class_to_idx:
                ...
                if species_labels:
                    ...
                    if classes:
                        ...
                        if hasattr(temp_model, "classifier") and hasattr(
                            temp_model.classifier, "linear"
                        ):
                            num_model_classes = (
                                temp_model.classifier.linear.out_features
                            )
                            if num_model_classes == 2:
                                class_names = ["Dominant Species", "Other Species"]
                                print(
                                    "Detected binary classification from model output size"
                                )
                            else:
                                class_names = [
                                    f"Class {i}" for i in range(num_model_classes)
                                ]
                        else:
                            # Ultimate fallback
    ...
    print(
        f"Loaded validation dataset with {len(val_dataset)} samples and {num_classes} classes"
    )
    ...
    for i in range(num_classes):
        ...
        if key not in report:
            ...
            if key not in report:
                print(
                    f"Warning: Could not find metrics for class {i} ({class_name}) in the report"
                )
